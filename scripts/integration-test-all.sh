...
## default port prefixes for bitommd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/bitommd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/bitommd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/bitommd add-genesis-account \
    "$("$PWD"/build/bitommd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/bitommd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/bitommd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/bitommd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/bitommd start --pruning=nothing --rpc.unsafe \
