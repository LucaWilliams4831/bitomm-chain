...
Find below a list of `bitommd` commands added with the `x/vesting` module. You can obtain the full list by using the `bitommd -h` command.
...
```go
bitommd add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
bitommd query vesting balances ADDRESS [flags]
```
...
```go
bitommd tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
bitommd tx vesting clawback ADDRESS [flags]
```
...
```go
bitommd tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
