...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.Acomposedenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    claimstypes.DefaultClaimsDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.Acomposedenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert bitomm", func() {
			// Register 'bitomm' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'bitomm' thru IBC
			...
			// 1. Send bitomm from Evmos to Osmosis
			...
			// check ibc bitomm coins balance on Osmosis
			...
			// 2. Send bitomm IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Acomposedenomtrace.Path, teststypes.Acomposedenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.poseIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc bitomm coins balance on Osmosis - should be zero
			...
			// check bitomm balance after transfer - should be equal to initial balance
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.poseIbcdenom)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialposeBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)
			...
			// should trigger claims logic and send bitomm coins from claims to receiver
			...
			receiverFinalposeBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, claimstypes.DefaultClaimsDenom)
			s.Require().Equal(receiverInitialposeBalance.Amount.Int64()+claimableAmount.Int64(), receiverFinalposeBalance.Amount.Int64())
		})
	})
