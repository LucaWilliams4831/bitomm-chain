...
Find below a list of  `bitommd` commands added with the  `x/erc20` module. You can obtain the full list by using the `bitommd -h` command. A CLI command can look like this:

```bash
bitommd query erc20 params
```
...
```bash
bitommd tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
bitommd tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
bitommd tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
