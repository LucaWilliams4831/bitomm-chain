...
Before upgrading the Evmos version. Stop your instance of `bitommd` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `bitommd` is running.
:::
...
```bash
rm $HOME/.bitommd/config/addrbook.json
