...
echo "export DAEMON_NAME=bitommd" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.bitommd" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.bitommd`) and copy over the current binary.

```bash
mkdir -p ~/.bitommd/cosmovisor
mkdir -p ~/.bitommd/cosmovisor/genesis
mkdir -p ~/.bitommd/cosmovisor/genesis/bin
mkdir -p ~/.bitommd/cosmovisor/upgrades

cp $GOPATH/bin/bitommd ~/.bitommd/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `bitommd` are the same:
...
```bash
sudo tee /etc/systemd/system/bitommd.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.bitommd"
Environment="DAEMON_NAME=bitommd"
...
sudo -S systemctl enable bitommd
sudo -S systemctl start bitommd
```

You can check the status with:

```bash
systemctl status bitommd
```
