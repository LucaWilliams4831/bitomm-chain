...
```bash
bitommd init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .bitommd/config folder

```bash
mv $HOME/genesis.json ~/.bitommd/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.bitommd/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.bitommd/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.bitommd/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.bitommd/config/config.toml
```

### Create bitommd service

```bash
echo "[Unit]
Description=bitommd Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which bitommd) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/bitommd.service; sudo mv $HOME/bitommd.service /etc/systemd/system/
```

```bash
sudo systemctl enable bitommd.service && sudo systemctl daemon-reload
```

### Run bitommd

```bash
sytemctl start bitommd
```

### Check logs

```bash
journalctl -u bitommd -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.bitommd/config/config.toml
```
