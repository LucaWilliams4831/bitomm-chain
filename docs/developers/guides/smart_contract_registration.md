...
bitommd tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
bitommd q revenue contract $CONTRACT
```
...
bitommd tx revenue update $CONTRACT $WITHDRAWER \
...
bitommd tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
