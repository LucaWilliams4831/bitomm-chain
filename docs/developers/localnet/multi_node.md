...
This command creates a 4-node network using the `composednode` Docker image.
...
Creating composednode0 ... done
Creating composednode2 ... done
Creating composednode1 ... done
Creating composednode3 ... done
```
...
calling the `bitommd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/bitommd` directory in each container.
...
```bash
# node 0: daemon logs
docker exec composednode0 tail bitommd.log
...
To interact with `bitommd` and start querying state or creating txs, you use the
`bitommd` directory of any given node as your `home`, for example:

```bash
bitommd keys list --home ./build/node0/bitommd
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/bitommd/key_seed.json` and can be restored to the CLI using the `bitommd keys add --restore` command
:::
