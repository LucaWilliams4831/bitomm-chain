...
```bash
$ bitommd keys add dev0-restored --recover
...
```bash
bitommd keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ bitommd keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ bitommd keys unsafe-import-eth-key dev0-imported ./dev0.export
